...
class PlacedObject(PandaNode):
    # generic object, no relation to collisions
    def __init__(self, loader: Loader, modelPath: str, parentNode: NodePath, nodeName: str):
        ...
        # making sure to have right type passed through
...
class CollidableObject(PlacedObject):
    # relate placedObject to collisions by using it as a helper class for this function to create collisions
    def __init__(self, loader: Loader, modelPath: str, parentNode: NodePath, nodeName: str):
        super(CollidableObject, self).__init__(loader, modelPath, parentNode, nodeName)

        self.collisionNode = self.modelNode.attachNewNode(CollisionNode(nodeName + "c_node"))
...
class CapsuleCollidableObject(CollidableObject):
    def __init__(self, loader: Loader, modelPath: str, parentNode: NodePath, nodeName: str, ax: float, ay: float, az: float, by: float, bx: float, bz: float, r: float):
